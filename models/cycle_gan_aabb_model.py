...
class CycleGANAABBModel(BaseModel):
    ...
    def backward_D_A(self):
        ...
        self.loss_D_A = self.backward_D_basic(self.netD_A, self.real_B1, fake_B1)

    def backward_D_B(self):
        fake_A1 = self.fake_A1_pool.query(self.fake_A1)
        self.loss_D_B =  self.backward_D_basic(self.netD_B, self.real_A1, fake_A1)
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            self.idt_A1 = self.netG_A.forward(self.real_B1, self.real_A2)
            self.loss_idt_A = self.criterionIdt(self.idt_A1, self.real_B1) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed.
            self.idt_B1 = self.netG_B.forward(self.real_A1, self.real_B2)
            self.loss_idt_B = self.criterionIdt(self.idt_B1, self.real_A1) * lambda_A * lambda_idt
        else:
            self.loss_idt_A = 0
            self.loss_idt_B = 0
        ...
        # D_A(G_A(A))
        self.fake_B1 = self.netG_A.forward(self.real_A1, self.real_B2)
        pred_fake = self.netD_A.forward(self.fake_B1)
        self.loss_G_A = self.criterionGAN(pred_fake, True)
        # D_B(G_B(B))
        self.fake_A1 = self.netG_B.forward(self.real_B1, self.real_A2)
        pred_fake = self.netD_B.forward(self.fake_A1)
